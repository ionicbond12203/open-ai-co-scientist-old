...
- **LLM Integration:** Uses OpenRouter or an OpenAI-compatible LM Studio server (model selection in UI).
...
3. **Choose an LLM backend in `.env`:**
    ...
    - Copy `.env.example` to `.env`, leave `LLM_PROVIDER=openrouter`, and set `OPENROUTER_API_KEY`.

    LM Studio:

    - Load at least one chat model and start LM Studio's local server.
    - In `.env`, set:
      ```dotenv
      ...
      LMSTUDIO_API_KEY=
      LMSTUDIO_MODEL=
      LMSTUDIO_REQUEST_TIMEOUT_SECONDS=300
      LMSTUDIO_CONNECT_TIMEOUT_SECONDS=10
      LMSTUDIO_ENABLE_FALLBACK=false
      CO_SCIENTIST_CYCLE_TIMEOUT_SECONDS=1800
      ```
    - The API key is optional unless LM Studio authentication is enabled. When
      `LMSTUDIO_MODEL` is empty, the UI discovers chat models from `/models` and
      prefers the smallest model whose size can be inferred from its ID.
    - LM Studio uses a short connection timeout and a longer inference timeout.
      Automatic model switching is disabled by default, so a pinned model stays pinned.
...
- Provider, endpoint, optional credential, and model can be set in `.env`.
...
- **Models:** Uses OpenRouter in hosted deployments or LM Studio for local/private inference.
...
This system requires either an OpenRouter API key or a reachable LM Studio server. The public demo uses a limited OpenRouter budget, so please use it responsibly.
