...
def test_lm_studio_model_list_bypasses_openrouter(gradio_app_module, monkeypatch):
    monkeypatch.setenv("LLM_PROVIDER", "lm_studio")
    monkeypatch.setattr(gradio_app_module, "CONFIGURED_LLM_MODEL", "")
    local_models = ["qwen/qwen3.5-9b", "qwen/qwen3.8-27b"]
    with (
        patch.object(gradio_app_module, "fetch_lm_studio_models", return_value=local_models) as mock_fetch,
        patch.object(gradio_app_module, "fetch_free_models") as mock_openrouter_fetch,
        patch.object(gradio_app_module.requests, "get") as mock_get,
    ):
        models = gradio_app_module.fetch_available_models()
        choices = gradio_app_module.get_model_dropdown_choices(models)

    assert models == local_models
    assert choices == local_models
    mock_fetch.assert_called_once()
