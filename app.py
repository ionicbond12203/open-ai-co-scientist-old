...
from app.config import config, get_configured_llm_model, get_llm_base_url, get_llm_provider
...
CONFIGURED_LLM_MODEL = get_configured_llm_model()
...
def fetch_available_models():
    """Fetch model choices from the configured LLM backend."""
    global available_models

    provider = get_llm_provider()
    configured_model = get_configured_llm_model(provider)
    if provider == "lm_studio":
        available_models = fetch_lm_studio_models()
        if not available_models:
            available_models = [configured_model or SAFE_FALLBACK_LLM_MODEL]
        logger.info("LM Studio: Found %d chat-capable models", len(available_models))
    ...
    try:
        ...
        logger.error("Failed to fetch models from OpenRouter: %s", redact_secrets(str(e)))
...
def get_default_model_choice(models: Optional[List[str]] = None) -> str:
    """Choose a provider-appropriate default model."""
    model_choices = models or available_models
    if get_llm_provider() == "lm_studio":
...
def get_model_dropdown_choices(models: Optional[List[str]] = None) -> List[str]:
    """Return model choices with the provider default first and de-duplicated."""
    model_choices = models or available_models
    choices = [get_default_model_choice(model_choices)]
    prioritized_models = (
        model_choices
        if get_llm_provider() == "lm_studio"
        else [*order_free_models_for_demo(model_choices), *model_choices]
    )
...
def get_deployment_status():
    ...
    if get_llm_provider() == "lm_studio":
        status = f"Local LM Studio | {len(available_models)} chat models | {get_llm_base_url('lm_studio')}"
        color = "green"
    elif is_hf_spaces:
        status = (
...
def create_gradio_interface():
    ...
    with gr.Blocks(
        ...
        .green { background-color: #d4edda; border: 1px solid #c3e6cb; }
        """,
    ) as demo:
        # Header
            ...
            with gr.Column(scale=2):
                ...
                with gr.Accordion("⚙️ Advanced Settings", open=False):
                    ...
                    model_dropdown = gr.Dropdown(
                        ...
                        info=(
                            "Models exposed by the configured LM Studio server."
                            if get_llm_provider() == "lm_studio"
        ...
        gr.HTML(
            ...
            <div style="text-align:center; margin-top: 30px;">
                <a href="https://github.com/ionicbond12203/open-ai-co-scientist-old" target="_blank" style="text-decoration:none; display:inline-flex; align-items:center; gap:8px;">
...
if __name__ == "__main__":
    # Check for API key
    if get_llm_provider() == "openrouter" and not os.getenv("OPENROUTER_API_KEY"):
    ...
    elif get_llm_provider() == "lm_studio":
        print(f"Using LM Studio at {get_llm_base_url('lm_studio')}")
